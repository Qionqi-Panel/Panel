...
var index = {
            ...
            obj.list.push({
                ...
                areaStyle: {
                    normal: {
                        ...
                        }, {offset: 1, color: 'rgba(255,70,131,.4'}], false)
                    }
                },
            ...
            obj.list.push({
                ...
                areaStyle: {
                    normal: {
                        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                            offset: 0,
                            color: 'rgba(108,192,207,1)'
                        }, {offset: 1, color: 'rgba(108,192,207,.4)'}], false)
                    }
                },
    ...
    get_init: function () {
        ...
        _this.reander_system_info(function (rdata) {
            ...
            $('#cpuChart').hover(function () {
                ...
                layer.tips(rdata.cpu[3] + "</br>" + rdata.cpu[5] + "个物理CPU，" + (rdata.cpu[4]) + "个物理核心，" + rdata.cpu[1] + "个逻辑核心</br>" + cpuText, this, {
                    time: 0,
                    tips: [1, '#999']
                });
            }, function () {
    ...
    reander_system_info: function (callback) {
        ...
        bt.system.get_net(function (res) {
            ...
            var disk_key = bt.get_cookie('disk_io_key') || 'ALL', disk_io_data = res.iostat[disk_key || 'ALL'],
                mb = 1048576,
            ...
            if (index.net.table) index.net.table.setOption({
                xAxis: {data: index.net.data.aData},
                series: [{name: lan.index.net_up, data: index.net.data.uData}, {
                    name: lan.index.net_down,
                    data: index.net.data.dData
                }]
            });
            if (index.iostat.table) index.iostat.table.setOption({
                xAxis: {data: index.iostat.data.aData},
                series: [{name: '读取字节数', data: index.iostat.data.uData}, {
                    name: '写入字节数',
                    data: index.iostat.data.dData
                }]
            });
    ...
    re_server: function () {
        ...
        setTimeout(function () {
            $('.rebt-con a').click(function () {
                ...
                switch (type) {
                    case 'panel':
                        layer.confirm(lan.index.panel_reboot_msg, {
                            title: lan.index.panel_reboot_title,
                            closeBtn: 2,
                            icon: 3
                        }, function () {
                            var loading = bt.load();
    ...
    get_simplify_time: function (dateTimeStamp) {
        ...
        var minute = 1000 * 60, hour = minute * 60, day = hour * 24, halfamonth = day * 15, month = day * 30,
            now = new Date().getTime(), diffValue = now - dateTimeStamp;
        if (diffValue < 0) return '刚刚';
        var monthC = diffValue / month, weekC = diffValue / (7 * day), dayC = diffValue / day, hourC = diffValue / hour,
    ...
    reader_warning_view: function () {
        ...
        function reader_warning_list(data) {
            var html = '', scan_time = '',
                arry = [['risk', '风险项'], ['security', '无风险项'], ['ignore', '已忽略项']],
                level = [['低危', '#e8d544'], ['中危', '#E6A23C'], ['高危', 'red']]
